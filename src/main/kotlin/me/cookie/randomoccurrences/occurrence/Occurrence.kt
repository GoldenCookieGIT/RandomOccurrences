...
abstract class Occurrence(
    ...
    private val randomOccurrences = occurrenceManager.plugin
    ...
    val bossBar = Bukkit.getServer()
            .createBossBar(
                randomOccurrences.placeholderManager
                    .replacePlaceholders("${randomOccurrences.messages.bossBarTitle} $friendlyName")
                    .formatHexColors(),
                BarColor.BLUE,
                BarStyle.SOLID
            )
    fun start() {
        ...
        Bukkit.getOnlinePlayers().forEach { player ->
            player.sendMessage(randomOccurrences.messages.header.formatHexColors())
            player.sendMessage("       ${randomOccurrences.messages.occurrenceStart
                    .replace("(friendlyName)", friendlyName)}".formatHexColors())
            description.forEach {
                player.sendMessage("    #4d4d4d$it".formatHexColors())
            }
            player.sendMessage("")
            player.sendMessage(randomOccurrences.messages.footer.formatHexColors())
        }
    ...
    fun end() {
        ...
        sortedMap.forEach { (uuid, score) ->
            ...
            player.sendMessage(randomOccurrences.messages.header.formatHexColors())
            player.sendMessage("       ${randomOccurrences.messages.occurrenceEnd
                    .replace("(friendlyName)", friendlyName)}".formatHexColors())
            player.sendMessage("")
            randomOccurrences.messages.occurrenceLeaderboard.forEach leaderboardLoop@{ leaderboardMsg ->
                if (leaderboardMsg.contains("(playerScore)")) {
                    if (place >= 4) {
                        randomOccurrences.messages.occurrenceLeaderboardPlayerScore.forEach {
    ...
    private fun startBossbar() {
        ...
        bossBarTask = object: BukkitRunnable() {
            override fun run() {
                ...
                bossBar.setTitle(
                        "${randomOccurrences.messages.bossBarTitle}&l$color$friendlyName".formatHexColors()
                )
